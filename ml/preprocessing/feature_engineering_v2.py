...
OUTPUT_FILE = Path("ml/data/processed/training_features_v2.csv")
...
def get_form_stats(history, window):
    """
    Calculate statistics from the team's most recent matches.

    IMPORTANT:
    The current match is NOT included in history yet.
    """
    ...
    return {
        ...
        "goals_scored": safe_average([x["goals_scored"] for x in recent]),
        "goals_conceded": safe_average([x["goals_conceded"] for x in recent]),
        "goal_difference": safe_average(
            [x["goals_scored"] - x["goals_conceded"] for x in recent]
        ),
        "shots": safe_average([x["shots"] for x in recent]),
        "shots_on_target": safe_average(
            [x["shots_on_target"] for x in recent]
        ),
        "corners": safe_average([x["corners"] for x in recent]),
    }


def get_venue_stats(history):
    """
    Calculate statistics specifically for home or away matches.
    """
    ...
    return {
        "win_rate": sum(x["points"] == 3 for x in history) / len(history),
        ...
        "ppg": safe_average([x["points"] for x in history]),
    }


def elo_expected(elo_home, elo_away):
    """Expected result for the home team using Elo."""

    difference = (elo_away - (elo_home + HOME_ELO_ADVANTAGE)) / 400
...
def main():
    ...
    print("Building Feature Engineering V2...")
    print("Features include:")
    print("  - Recent form (3 / 5 / 10)")
    print("  - Goals scored / conceded")
    ...
    for _, match in df.iterrows():
        ...
        home_form = {
            window: get_form_stats(team_history[home], window)
            for window in FORM_WINDOWS
        }

        away_form = {
            window: get_form_stats(team_history[away], window)
            for window in FORM_WINDOWS
        }
        ...
        home_venue = get_venue_stats(home_history[home])
        away_venue = get_venue_stats(away_history[away])
        ...
        row = {
            ...
            "home_goals_scored_3": home_form[3]["goals_scored"],
            "home_goals_scored_5": home_form[5]["goals_scored"],
            "home_goals_scored_10": home_form[10]["goals_scored"],

            "home_goals_conceded_3": home_form[3]["goals_conceded"],
            "home_goals_conceded_5": home_form[5]["goals_conceded"],
            "home_goals_conceded_10": home_form[10]["goals_conceded"],

            "home_goal_diff_5": home_form[5]["goal_difference"],
            "home_goal_diff_10": home_form[10]["goal_difference"],

            "home_shots_avg": home_form[5]["shots"],
            "home_shots_on_target_avg": home_form[5]["shots_on_target"],
            "home_corners_avg": home_form[5]["corners"],
            ...
            "away_win_rate_3": away_form[3]["win_rate"],
            "away_win_rate_5": away_form[5]["win_rate"],
            "away_win_rate_10": away_form[10]["win_rate"],

            "away_goals_scored_3": away_form[3]["goals_scored"],
            "away_goals_scored_5": away_form[5]["goals_scored"],
            "away_goals_scored_10": away_form[10]["goals_scored"],

            "away_goals_conceded_3": away_form[3]["goals_conceded"],
            "away_goals_conceded_5": away_form[5]["goals_conceded"],
            "away_goals_conceded_10": away_form[10]["goals_conceded"],

            "away_goal_diff_5": away_form[5]["goal_difference"],
            "away_goal_diff_10": away_form[10]["goal_difference"],

            "away_shots_avg": away_form[5]["shots"],
            "away_shots_on_target_avg": away_form[5]["shots_on_target"],
            "away_corners_avg": away_form[5]["corners"],
            ...
            "home_home_win_rate": home_venue["win_rate"],
            "home_home_goals_avg": home_venue["goals_scored"],
            "home_home_conceded_avg": home_venue["goals_conceded"],
            "home_home_ppg": home_venue["ppg"],

            "away_away_win_rate": away_venue["win_rate"],
            "away_away_goals_avg": away_venue["goals_scored"],
            "away_away_conceded_avg": away_venue["goals_conceded"],
            "away_away_ppg": away_venue["ppg"],
    ...
    print(
        f"Columns generated: {len(features_df.columns)}"
    )

    print(
        f"Missing values: {features_df.isna().sum().sum()}"
    )
    ...
    print(features_df["FTR"].value_counts())
